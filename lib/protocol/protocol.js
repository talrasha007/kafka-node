        ...
        Binary.parse(messageSet)
            ...
            .tap(function (vars) {
                if (vars.value !== -1) {
                    ...
                    switch (vars.attributes) {
                        case CODEC_NONE:
                            vars.value = vars.value.toString();
                            break;
                        case CODEC_GZIP:
                            if (zlib.gunzipSync) {
                                var data = zlib.gunzipSync(vars.value);
                                set = decodeMessageSet(topic, partition, data, cb, maxTickMessages);
                            } else {
                                // Async zlib.gunzip may cause commit issue, data may be processed after commit.
                                set.push(vars.offset);
                                return zlib.gunzip(vars.value, function (err, data) {
                                    if (err) cb(err);
                                    else decodeMessageSet(topic, partition, data, cb, maxTickMessages);
                                });
                            }
                        case CODEC_SNAPPY:
                            // TODO: SNAPPY support.
